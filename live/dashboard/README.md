...
Read-only "live tape" for the Track B paper-trading bot. Three pieces:
...
layer bolted onto an existing sqlite DB; if it breaks, the bot keeps trading.
...
```sh
sudo cp live/dashboard/sync/algo-traders-sync.env.example /etc/algo-traders-sync.env
sudo chmod 600 /etc/algo-traders-sync.env
sudo nano /etc/algo-traders-sync.env   # paste SUPABASE_SERVICE_KEY, fix paths
sudo bash live/dashboard/sync/install.sh
```
...
The bot has its own healthcheck pipeline via `live/ops/healthcheck.sh` — this
