...
> **Queue:** Bootstrap the MDD CI; relay the TON delisting to Ethan; keep running `forward_accumulate.py` daily to genuinely accumulate observations toward N=250. Separately: `aurora-forecaster/` needs its four text sources aligned into one input for the unimodal-vs-multimodal comparison. Also check for the literature-search workflow's first-ever PR (`gh run 31125217798`, triggered 2026-08-06).
