...
*All code, result cards, and the Pareto chart in this piece are reproducible from the public repo. The chart is generated by `scripts/generate_pareto_chart.py`; the DSR analysis by `scripts/dsr_analysis.py`; the strategies live in `user_data/strategies/`; the per-experiment result cards are in `wiki/results/`. None of this is investment advice — I'm a researcher with a laptop, and you have no idea what your future self is going to need.*
