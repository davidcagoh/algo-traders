...
**Strategy file:** `strategies/HmmSmaSlopeV2.py`
...
V1 (HmmSmaSlope) used a binary slope gate (`sma180_slope > 0` blocks entry) and cost 15pp of bull return vs unfiltered HMM-multi. The card's next-test list flagged continuous position sizing per `decision 004` as the natural successor: scale stake by slope strength rather than skip the trade.
...
Archives: `analysis/backtests/hmm_sma_slope_v2_binance_bull.zip`, `…_hl_bear.zip`.
