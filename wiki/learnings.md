...
3. **First real-data Pareto datapoint (2026-05-09):** `HmmRegime4` backtest result (`backtesting/wiki/results/2026-05-09-hmm-regime-4.md`) is the first concrete case where two strategies on the same project disagree across the proposed Pareto objectives. SmaRegime180 wins on Profit Factor (2.72 vs 1.58); HmmRegime4 wins on win rate (45.9% vs 21.9%), MDD (1.03% vs 1.74%), SQN (1.38 vs 1.02), trade density (74 vs 32). Neither dominates. Total return is approximately equal (+2.65% vs +2.83%). This is exactly the structure the multi-objective frame predicts: each strategy is a different *corner* of the Pareto frontier, and the right answer is to run both (with kill rules) and let the meta-allocator weight by recent regime fit, not pick a "winner."
   - **Caveat:** HmmRegime4 has a known look-ahead caveat (HMM fit on full visible window). The honest comparison requires the rolling-window refit (queued as backtesting next-experiment #1).
   - **First sweep candidate:** when the data-precondition gate clears (per `methodology/multi-objective-search.md`), the SMA-family + HMM-family + carry-family across the 7 majors is the natural first NSGA-II run.
