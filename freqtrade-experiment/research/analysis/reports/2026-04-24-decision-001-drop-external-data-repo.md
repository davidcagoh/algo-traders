...
Remove the `freqtrade_hyperliquid_download-data` gitlink and use freqtrade's native `download-data` into `freqtrade/data/hyperliquid/` instead.
...
- We lose whatever historical candles Ethan had stockpiled in that separate repo. He can't find how he got them, so they weren't reproducible anyway. See `analysis/reports/2026-04-24-decision-002-hyperliquid-deep-history.md` for how to rebuild deep history if needed.
