...
```sh
sudo cp freqtrade-experiment/monitoring/sync/algo-traders-sync.env.example /etc/algo-traders-sync.env
sudo chmod 600 /etc/algo-traders-sync.env
sudo nano /etc/algo-traders-sync.env   # paste SUPABASE_SERVICE_KEY, fix paths
sudo bash freqtrade-experiment/monitoring/sync/install.sh
```
...
```sh
cd freqtrade-experiment/monitoring/web
...
The bot has its own healthcheck pipeline via `freqtrade-experiment/execution/ops/healthcheck.sh` — this
