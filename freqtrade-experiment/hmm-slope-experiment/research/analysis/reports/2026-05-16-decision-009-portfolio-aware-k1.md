...
**Status:** decided
**Date:** 2026-05-16
**Supersedes:** none. Refines K1 from `2026-05-10-decision-004-kill-criteria-sma-regime-180.md` for portfolio-addition use.
...
The K1 = 5.5% hard-MDD ceiling defined in `decision 004` is breached by 0.55pp. Two readings of the same number:
...
K1 = 5.5% was derived in `decision 004` as `3 × IS_MDD(T3, Hyperliquid bear)` = `3 × 1.74%` ≈ 5.5%. The multiplier captured the empirical fact that live MDD typically exceeds IS MDD by ~3× under regime shift.
...
- **Robust MDB** (positive under equal-weight, risk-parity, and mean-var) follows the definition pre-registered in `decision 005` §7 and operationalised in `methodology/correlation-and-mdb.md`. MDB-rp is the headline; the eq/mv flank checks prevent gaming a single weighting scheme.
...
- **Companion (upstream):** `2026-05-10-decision-004-kill-criteria-sma-regime-180.md` — defines `K1_standalone = 5.5%` and the IS-MDD-multiplier derivation.
- **Companion (methodology):** `2026-05-16-decision-005-evaluation-and-diversity-plan.md` §7 — pre-registers MDB-eq/rp/mv, the robust flag, and the common-window basis.
- **Companion (operationalisation):** `methodology/correlation-and-mdb.md` — loader assumptions and the eq/rp/mv definitions.
- **Trigger:** `2026-05-16-decision-010-paper-plan-deferred.md` item #3 of the prerequisite list — this decision was the named blocker.
- **Upstream methodology:** [`../../../../../wiki/concepts/kill-criteria.md`](../../../../../wiki/concepts/kill-criteria.md) (meta-wiki, 2026-05-16) — the "revise-vs-kill review" section and the "portfolio-justified breach" exception name this decision as the canonical specification of the portfolio-aware rule.
