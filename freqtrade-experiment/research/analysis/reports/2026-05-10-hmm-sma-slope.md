...
**Strategy file:** `strategies/HmmSmaSlope.py`
**Data:** Binance USDT-margined perp 6-coin basket (bull) + Hyperliquid USDC 7-coin basket (bear)
**Config:** `configs/config_binance_multi.json` (bull), `configs/config_hl_multi.json` (bear)
**Fee:** `--fee 0.00035`
...
3. **Position-size scaling instead of binary gate.** Per `2026-05-10-decision-004-kill-criteria-sma-regime-180.md`'s continuous-shrinkage formula, the right move may be to *shrink* size as slope weakens rather than skip entry entirely. Tests whether the gate's binary cliff is leaving signal on the table.
...
# Bear window
./.venv/bin/freqtrade backtesting \
  --datadir data --strategy-path strategies -c configs/config_hl_multi.json \
  --data-format-ohlcv feather -s HmmSmaSlope -i 4h \
  --fee 0.00035 --eps --max-open-trades 7 \
  --timerange 20251015-20260509 --export trades
```

Result archives: `analysis/backtests/hmm_sma_slope_binance_bull.json` and `…_hl_bear.json`.
