...
- Verify the new `forward-accumulation.yml` GitHub Action actually works end-to-end: trigger a manual `workflow_dispatch` run, confirm it pushes an updated `forward_state.json`/`forward_daily_log.jsonl` commit to `main`, and confirm the healthchecks.io dashboard shows a received ping (secret `HEALTHCHECKS_FORWARD_ACCUMULATION_URL` just set, unverified). This replaces the old manual-daily-run item — the underlying accumulation bug (window-sliding froze N at 148 forever; see `learnings-archive.md` 2026-08-07) is fixed and now automated via cron `30 1 * * *` UTC. Once confirmed, no further manual intervention needed toward the N=250 DSR-binding floor (~102 more days from N=148).
...
- Is aurora-forecaster's severe calibration miss (nominal 50/80/95% coverage empirically only ~20/28/38%, see `learnings-archive.md` 2026-08-07) a real property of the model at a 96h horizon, or an artifact of only 4 scored origins / a specific BTC regime window? Revisit once `rolling_forecast_btc.py` accumulates more origins.
