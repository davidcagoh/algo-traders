...
**Script:** `analysis/dsr_analysis.py`
**Inputs:** Nine backtest archives in `analysis/backtests/` covering five strategies × two windows each
...
Reads all archives listed in `RUNS` at the top of the script; outputs to `analysis/reports/_dsr_table.json` and prints summary tables.
