...
- **Inter-strategy correlation becomes a first-class metric.** A new `analysis/eval_layers.py` produces a daily-return matrix across all leaderboard ZIPs and reports pairwise correlation + marginal-diversification-benefit (MDB) per candidate. R∧T1/V2/V3 will almost certainly collapse to one effective strategy under this lens; that's the point.
...
**Deliverable.** New module `analysis/eval_layers.py` exposing `compute_layer5_metrics(zip_path: Path) -> dict`:
...
**Steps.**
1. Extract `load_daily_returns` + `load_trade_returns` from `analysis/dsr_analysis.py:73-100` into `analysis/eval_layers.py`. Shared loader.
2. Add the seven functions, each <15 lines, pure functions of a `pd.Series`.
3. CLI `python analysis/eval_layers.py <zip_path>` prints a markdown table to stdout.
4. Update `analysis/reports/_template.md:15-27` with a "Layer 5 — Tail/Path shape" sub-table.
5. Backfill the 9 leaderboard runs in `dsr_analysis.py:46-56`. Insert tables into existing result cards — do not rewrite prose.

**Exit criteria.** All 9 result cards have a Layer-5 table. `_index.md` leaderboard at `../_index.md:55-66` gains one new column: **Ulcer Index** (Martin is derivative). One paragraph in `learnings.md` records whether Ulcer disagrees with MDD anywhere — specifically, *does R2 still look killable under Ulcer or did MDD's point-in-time-ness exaggerate today's K1 breach?*

**Dependencies.** None. **Risk.** Low.

### A2 — Inter-strategy correlation matrix (1–2 weeks)

**Deliverable.** `analysis/eval_layers.py` gains `build_corr_matrix(zip_paths, common_window) -> pd.DataFrame` + `marginal_diversification_benefit(candidate, existing_book) -> float`.

**Steps.**
1. **Common window** = intersection of daily-wallet date ranges. Refuse to compute on overlap <90 days. Missing day = flat = 0 return.
2. Pairwise Pearson + Spearman on daily wallet log-returns. Output `analysis/reports/correlation_matrix.png` (heatmap, family-colored labels) and `analysis/reports/_correlation_table.json` (machine-readable, mirroring `dsr_analysis.py:189-208`).
...
4. Add an **MDB column** to `../_index.md:55` leaderboard. This is the single most important new column — it justifies (or kills) R∧T2/V3 under correlation-aware evaluation.

**Exit criteria.**
- Heatmap rendered, embedded in `README.md` alongside Pareto chart.
- Each leaderboard row shows `corr_to_book` and `MDB`.
- `../../../../wiki/concepts/correlation-and-mdb.md` (~80 lines) documents loader assumptions, citing `multi-objective-search.md`'s OOS-stability objective as theoretical precedent.
...
- `strategies/PairsZScore.py`: enter at |z| > 2 of (log P_A − β log P_B); exit at z = 0 or stop at |z| > 4 or time-stop = 3 × half-life. β refit weekly on prior 60d only.
...
- Pre-register `2026-05-16-decision-006-kill-criteria-pairs.md` before backtest: MDD > 8%, half-life drift >3× rolling median, cointegration p > 0.20 for 30 consecutive days.
...
**Exit criteria.** Standalone + MDB on leaderboard. Drop if MDB < 0.05 vs (T3 ∪ B1). Pre-registered kill criteria in `2026-05-16-decision-007-kill-criteria-cross-sectional.md`.

**Dependencies.** A2. **Risk.** Low.

### B3 — Funding-extreme mean-reversion (1 week)

**Spec.** Implements `learnings.md` #10 + #11. OU-calibrate funding series per coin from `data/hyperliquid/funding/`. Entry when funding z-score > 2: **counter-funding** position (short if funding pays longs), time-exit at 3 bars (4h).

**Exit criteria.** Standalone + MDB. Pre-registered MDD > 4% in `2026-05-16-decision-008-kill-criteria-funding-mr.md`.
...
- **Pre-registration ledger.** Each strategy links to `decision 00N-kill-criteria-<strategy>.md`. Strategies without one flagged "pre-DSR era — research only."
...
Week 5:    B1.1 — cointegration preflight + decision 006
...
A1.5 deliverable: 11 new ZIPs under `analysis/backtests/`, one summary table appended to `_index.md` as "Binance 5-coin common-window leaderboard" alongside the existing Hyperliquid-bear leaderboard. Result cards updated with a `## Binance 5-coin re-run` section (do not overwrite original Hyperliquid sections).
...
5. **Pre-registration discipline** = **non-negotiable**. Every new strategy gets `decision 00N-kill-criteria-<strategy>.md` *before* its first backtest, following the `decision 004` template. No exceptions.
...
- `analysis/dsr_analysis.py:73-100, 152, 189-208` — share loader, share annualization, share JSON output pattern
- `analysis/generate_pareto_chart.py:43-56, 62-123, 126-143` — add status field, frontier rule, MDB panel
- `../_index.md:55-66` — leaderboard column additions (Ulcer, MDB)
- `analysis/reports/_template.md:15-27` — add Layer-5 sub-table
- `analysis/reports/2026-05-10-decision-004-kill-criteria-sma-regime-180.md` — template for kill-criteria pre-registration
- `../../../../wiki/concepts/multi-objective-search.md:31-43` — Pareto-objectives rationale
- `../../../../wiki/concepts/cv-and-deflation.md:44-69` — DSR/PBO/held-out gate

New files this plan creates:
- `analysis/eval_layers.py`
- `scripts/cointegration_preflight.py`
- `strategies/PairsZScore.py`
- `strategies/CrossSectionalMomentum.py`
- `strategies/FundingExtremeMR.py`
- `../../../../wiki/concepts/correlation-and-mdb.md`
- `analysis/reports/2026-05-16-decision-006-kill-criteria-pairs.md`
- `analysis/reports/2026-05-16-decision-007-kill-criteria-cross-sectional.md`
- `analysis/reports/2026-05-16-decision-008-kill-criteria-funding-mr.md`
