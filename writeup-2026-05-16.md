...
Full methodology: `backtesting/wiki/methodology/correlation-and-mdb.md`.
...
This proposal is in `wiki/decisions/006-kill-criteria-pairs.md` as a precedent (K1-pairs is set at 8% deliberately); the formal cross-strategy update lives in a forthcoming `decisions/009-kill-criteria-portfolio.md`.
...
![Pareto chart](backtesting/wiki/assets/pareto.png)
...
![Correlation matrix](backtesting/wiki/assets/correlation_matrix.png)
...
*Generated 2026-05-16 in a single autonomous session over A1 → A1.5 → A2 → C → B1 → B2 → B3 → D. See `backtesting/wiki/decisions/005-evaluation-and-diversity-plan.md` for the plan that produced this writeup.*
