...
The book is the set of strategies currently tagged ★ (paper-trade candidate) in `analysis/reports/reference/strategy-taxonomy.md`. As of 2026-05-16:
...
Code: `analysis/eval_layers.py` — functions `build_returns_matrix`, `correlation_matrix`, `marginal_diversification_benefit`, `mdb_robust_flag`, `render_heatmap`.

Driver: `analysis/run_correlation_mdb.py` — reads all 12 A1.5 ZIPs, emits `analysis/reports/correlation_matrix.png` + `analysis/reports/_correlation_table.json`.

Run after each new strategy backtest:

```shell
./.venv/bin/python analysis/run_correlation_mdb.py
```
