...
- Decide whether `evaluation-framework/` is primarily a methods paper, a reusable package/practitioner release, or both; then reconcile every public claim with the failed `HmmSmaSlopeV2` live run.
- Preserve the 2026-06-01 through 2026-12-31 evaluation window as a one-shot gate; do not inspect a partial window while deciding the manuscript's direction.
...
- Is the evaluation framework's strongest contribution the six-layer API, the portfolio-aware kill criterion, or the worked example's honest backtest-to-live falsification?
- Is there a strategy hypothesis strong enough to justify a new experiment after the failed Freqtrade paper run?
- Should CPCV with embargo become the default sweep protocol, and what minimum sample requirements make it informative?
