...
2. **Reconcile the completed live test.** Only `HmmSmaSlopeV2` was deployed,
   not the full proposed book. Its extended run failed the live gate: 21 closed
   trades, -3.54%, 4.8% win rate, and an eleven-loss streak. The manuscript
   still describes this as future work and must be revised before release.
...
`../../freqtrade-experiment/hmm-slope-experiment/research/analysis/reports/`. These are the per-experiment evidence the
paper cites.

See [`../STATUS.md`](../STATUS.md) before changing the manuscript's claims.
