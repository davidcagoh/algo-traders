...
`live/wiki/decisions/002-track-b-gate.md`.

Three pieces, no more:

1. **Docker Compose** — Freqtrade container with `restart: unless-stopped`.
   Built from the pinned in-repo source (commit `9d0fb9b`), not the upstream
   Docker Hub image, so the binary matches the gate version pin.
...
cp ../tracks/hl_paper/freqtrade/config.private.example.json \
   ../tracks/hl_paper/freqtrade/config.private.json
$EDITOR ../tracks/hl_paper/freqtrade/config.private.json
...
| Gate criterion (decisions/002) | How this layer satisfies it                          |
|--------------------------------|------------------------------------------------------|
...
Any code or config change inside the 30d window invalidates the gate — restart
the clock and record the reason in `live/wiki/results/`.
